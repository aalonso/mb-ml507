...
static XUartLite XUartInstance;
...
int TftExample(u32 TftDeviceId)
{
	...
	/* Initialize RS232_Uart_1 
	 * Get address of the XUartLite_Config structure for the given device id.
	 */
    XUartConfigPtr = XUartLite_LookupConfig(XPAR_UARTLITE_0_DEVICE_ID);
    if (XUartConfigPtr == (XUartLite_Config *)NULL) {
		return XST_FAILURE;
	}
			
    Status = XUartLite_CfgInitialize(&XUartInstance, XUartConfigPtr, 
	...
	framebuf_ptr = (int*)XPAR_DDR2_SDRAM_MPMC_BASEADDR;
